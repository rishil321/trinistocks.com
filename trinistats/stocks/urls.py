...
urlpatterns = [
    ...
    path('userprofile', views.UserProfileView.as_view(),
        name='userprofile'),
    path("account/password_reset", views.PasswordResetRequestView.as_view(), name="password_reset_request"),
    path('account/password_reset_confirm/<uidb64>/<token>', auth_views.PasswordResetConfirmView.as_view(template_name="stocks/account/base_passwordresetconfirm.html",success_url=reverse_lazy('stocks:password_reset_complete',current_app="stocks")), name='password_reset_confirm'),
    path('account/password_reset_complete', auth_views.PasswordResetCompleteView.as_view(template_name='stocks/account/base_passwordresetcomplete.html'), name='password_reset_complete'),
    path('account/password_change', auth_views.PasswordChangeView.as_view(template_name='stocks/account/base_passwordchange.html',success_url=reverse_lazy('stocks:password_change_complete',current_app="stocks")), name='password_change'),
    path('account/password_change_complete', auth_views.PasswordChangeDoneView.as_view(template_name='stocks/account/base_passwordchangecomplete.html'), name='password_change_complete'),
]
