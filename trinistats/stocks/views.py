...
def dailyequitysummary(request):
    try:
        ...
        if request.method == 'GET' and all(x in request.GET for x in ['selecteddate']):
            ...
            selecteddate = models.Dailyequitysummary.objects.latest('date').date
        ...
        dailyequitysummaryrecords = models.Dailyequitysummary.objects.filter(date = selecteddate).select_related('equityid').order_by('-valuetraded')
        ...
        selectedrecords = dailyequitysummaryrecords.annotate(symbol=F('equityid__symbol'),securityname=F('equityid__securityname')).values('securityname','symbol','valuetraded','volumetraded','saleprice','low','high','changedollars')
        # Set up our table 
        ...
        graphvaluetraded = [record['valuetraded'] for record in selectedrecords[:10]]
...
def stockhistory(request):
    try:
        ...
        if request.method == 'GET' and all(x in request.GET for x in ['startdate','enddate','equityid']):
            ...
            enteredstartdate = (datetime.now()+dateutil.relativedelta.relativedelta(months=-3)).strftime('%Y-%m-%d')
            enteredenddate = datetime.now().strftime('%Y-%m-%d')
        if request.GET.get('sort'):
            orderby=request.GET.get('sort')
        else:
            orderby = 'date'
        selectedstock = models.Listedequities.objects.get(equityid=selectedequityid)
        # Fetch the records
        historicalrecords = models.Historicalstockinfo.objects.filter(equityid=selectedequityid).filter(date__gt=enteredstartdate).filter(date__lt=enteredenddate)
        # Set up our table 
        tabledata = HistoricalStockInfoTable(historicalrecords, order_by=orderby)
        tabledata.paginate(page=request.GET.get("page", 1), per_page=10)
        # Set up our graph
        graphlabels = [obj.date.strftime('%Y-%m-%d') for obj in historicalrecords]
        graphdataset = []
...
def dividendhistory(request):
    try:
        ...
        if request.method == 'GET' and all(x in request.GET for x in ['startdate','enddate','equityid']):
            ...
            # These will be overridden later on if the user has submitted input data
            selectedequityid = models.Listedequities.objects.first().equityid
            enteredstartdate = (datetime.now()+dateutil.relativedelta.relativedelta(years=-10)).strftime('%Y-%m-%d')
            enteredenddate = datetime.now().strftime('%Y-%m-%d')
        if request.GET.get('sort'):
            orderby=request.GET.get('sort')
        else:
        ...
        historicalrecords = models.Historicaldividendinfo.objects.filter(equityid=selectedequityid).filter(recorddate__gt=enteredstartdate).filter(recorddate__lt=enteredenddate)
        if not historicalrecords:
            errors += "No data found for equity and date range selected."
        # Set up our table 
        tabledata = HistoricalDividendInfoTable(historicalrecords, order_by=orderby)
        tabledata.paginate(page=request.GET.get("page", 1), per_page=10)
        # Set up our graph
        graphlabels = ""
        graphdataset = []
        graphlabels = [obj.recorddate.strftime('%Y-%m-%d') for obj in historicalrecords]
        # Add the composite totals
...
def dividendyieldhistory(request):
    try:
        ...
        selectedstock = models.Listedequities.objects.get(equityid=selectedequityid)
        # Fetch some default records
        historicalrecords = models.Dividendyield.objects.filter(equityid=selectedequityid).filter(yielddate__gt=enteredstartdate).filter(yielddate__lt=enteredenddate)
        if not historicalrecords:
            errors += "No data found for equity and date range selected."
        # Set up our table 
        tabledata = HistoricalDividendYieldTable(historicalrecords, order_by=orderby)
        tabledata.paginate(page=request.GET.get("page", 1), per_page=10)
        # Set up our graph
        graphlabels = ""
        graphdataset = []
        graphlabels = [obj.yielddate.strftime('%Y-%m-%d') for obj in historicalrecords]
        # Add the composite totals
...
def markethistory(request):
    try:
        ...
        if request.GET.get('sort'):
            ...
            orderby = 'date'
        # Fetch the records
        historicalrecords = models.Historicalmarketsummary.objects.filter(date__gt=enteredstartdate).filter(date__lt=enteredenddate)
        # Set up our table 
        tabledata = HistoricalMarketSummaryTable(historicalrecords, order_by=orderby)
        tabledata.paginate(page=request.GET.get("page", 1), per_page=10)
        # Set up our graph
        graphlabels = [obj.date.strftime('%Y-%m-%d') for obj in historicalrecords]
        graphdataset = []
...
def ostradeshistory(request):
    try:
        ...
        selectedstock = models.Listedequities.objects.get(equityid=selectedequityid)
        # Fetch the records
        selectedrecords = models.Dailyequitysummary.objects.filter(equityid=selectedequityid).filter(date__gt=enteredstartdate).filter(date__lt=enteredenddate).values('date','osbid','osbidvol','osoffer','osoffervol').order_by('date')
        # Set up our table 
        tabledata = OSTradesHistoryTable(selectedrecords, order_by=orderby)
        tabledata.paginate(page=request.GET.get("page", 1), per_page=10)
        # Set up our graph
        graphlabels = [obj['date'].strftime('%Y-%m-%d') for obj in selectedrecords]
        graphdataset = []
        # Add the composite totals
