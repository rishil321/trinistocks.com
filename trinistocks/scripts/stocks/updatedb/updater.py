...
def fetch_latest_currency_conversion_rates():
    ...
    api_response_ttd = requests.get(
        url=
        "https://fcsapi.com/api-v2/forex/base_latest?symbol=TTD&type=forex&access_key=o9zfwlibfXciHoFO4LQU2NfTwt2vEk70DAiOH1yb2ao4tBhNmm"
    )
...
def calculate_fundamental_analysis_ratios(TTD_JMD, TTD_USD, TTD_BBD):
    ...
    try:
        for raw_data_table_name in raw_data_table_names:
            with DatabaseConnect() as db_connect:
                ...
                share_price_df = pd.io.sql.read_sql(
                    ...
                    db_connect.dbengine)
                # create a merged df
                calculated_ratios_df = pd.merge(raw_data_df,
                                                share_price_df,
                                                how='outer',
                                                on='symbol')
                # set the report type
                ...
                calculated_ratios_df['RoE'] = calculated_ratios_df['net_income'] / \
                    calculated_ratios_df['total_shareholders_equity']
                # now calculate the return on invested capital
                calculated_ratios_df['RoIC'] = calculated_ratios_df['profit_after_tax'] / \
                    calculated_ratios_df['total_shareholders_equity']
                # now calculate the working capital
                calculated_ratios_df['working_capital'] = calculated_ratios_df['total_assets'] - \
                    calculated_ratios_df['total_liabilities']
                # and the current ratio
                calculated_ratios_df['current_ratio'] = calculated_ratios_df['total_assets'] / \
                    calculated_ratios_df['total_liabilities']
                calculated_ratios_df[
                    'share_price_conversion_rates'] = calculated_ratios_df.apply(
                        lambda x: TTD_USD if x.currency == 'USD' else
                        (TTD_JMD if x.currency == 'JMD' else
                         (TTD_BBD if x.currency == 'BBD' else 1.00)),
                        axis=1)
                calculated_ratios_df['price_to_earnings_ratio'] = (calculated_ratios_df['close_price'] * calculated_ratios_df['share_price_conversion_rates']) / \
                    calculated_ratios_df['basic_earnings_per_share']
                # calculate cash per share
                calculated_ratios_df['cash_per_share'] = (calculated_ratios_df['cash_cash_equivalents']) / \
                ...
                calculated_ratios_df[
                    'dividend_conversion_rates'] = calculated_ratios_df.apply(
                        lambda x: 1 / TTD_USD
                        if x.symbol in USD_DIVIDEND_SYMBOLS else
                        (1 / TTD_JMD if x.symbol in JMD_DIVIDEND_SYMBOLS else
                         (1 / TTD_BBD
                          if x.symbol in BBD_DIVIDEND_SYMBOLS else 1.00)),
                        axis=1)
                # now calculate a conversion rate for the price for the dividend yields
                calculated_ratios_df[
                    'dividend_stock_price_conversion_rates'] = calculated_ratios_df.apply(
                        lambda x: 1 / TTD_USD
                        if x.symbol in USD_STOCK_SYMBOLS else 1.00,
                        axis=1)
                # note that the price_to_earnings_df contains the share price
                calculated_ratios_df['dividend_yield'] = 100 * \
                    (calculated_ratios_df['dividends_per_share']*calculated_ratios_df['dividend_conversion_rates']) / \
                    (calculated_ratios_df['close_price'] *
                     calculated_ratios_df['dividend_stock_price_conversion_rates'])
                # now dividend payout ratio
                calculated_ratios_df['dividend_payout_ratio'] = 100 * calculated_ratios_df['dividends_per_share'] / \
                    calculated_ratios_df['basic_earnings_per_share']
                ...
                calculated_ratios_df['PEG'] = calculated_ratios_df['price_to_earnings_ratio'] / \
                    calculated_ratios_df['EPS_growth_rate']
                # calculate the book value per share (BVPS)
                calculated_ratios_df['book_value_per_share'] = (calculated_ratios_df['total_assets'] - calculated_ratios_df['total_liabilities']) / \
                    calculated_ratios_df['total_shares_outstanding']
                # calculate the price to book ratio
                calculated_ratios_df['price_to_book_ratio'] = (calculated_ratios_df['close_price'] * calculated_ratios_df['share_price_conversion_rates']) / \
                    ((calculated_ratios_df['total_assets'] - calculated_ratios_df['total_liabilities']) /
                     calculated_ratios_df['total_shares_outstanding'])
                ...
                # rename the columns
                calculated_ratios_df.rename(columns={
                    'year_end_date': 'date',
                ...
                while not execute_completed_successfully and execute_failed_times < 5:
                    try:
                        insert_stmt = insert(
                            calculated_fundamental_ratios_table).values(
                                calculated_ratios_df.to_dict('records'))
        ...
        logger.exception("Could not complete fundamental data update.",
                         exc_info=exc)
...
def update_dividend_yields(TTD_JMD, TTD_USD, TTD_BBD):
    ...
    with DatabaseConnect() as db_connect:
        ...
        share_price_df['share_price_conversion_rates'] = share_price_df.apply(
            lambda x: 1 / TTD_USD
            if x.currency == 'USD' else (1 / TTD_JMD
                                         if x.currency == 'JMD' else
                                         (1 / TTD_BBD
                                          if x.currency == 'BBD' else 1.00)),
            axis=1)
        # merge this df with the dividends df
        yearly_dividends_df = pd.merge(share_price_df,
                                       yearly_dividends_df,
                                       how='inner',
                                       on='symbol')
        ...
        yearly_dividends_df.drop(columns=[
            'close_price', 'currency', 'share_price_conversion_rates',
            'dividend_amount', 'dividend_conversion_rates'
        ],
                                 inplace=True)
        # format the dates properly
        yearly_dividends_df.rename(columns={'record_date': 'date'},
                                   inplace=True)
...
def update_portfolio_summary_book_costs():
    ...
    try:
        with DatabaseConnect() as db_connect:
            ...
            total_shares_bought_df = transactions_df[
                transactions_df.bought_or_sold == "Bought"].groupby(
            ...
            total_shares_sold_df = transactions_df[
                transactions_df.bought_or_sold == "Sold"].groupby(
                    ['user_id', 'symbol_id']).sum().reset_index()
            total_shares_sold_df.drop(['share_price'], axis=1, inplace=True)
            total_shares_sold_df.rename(columns={'num_shares': 'shares_sold'},
                                        inplace=True)
            ...
            avg_cost_df = transactions_df[transactions_df.bought_or_sold ==
                                          "Bought"].copy()
...
def update_portfolio_summary_market_values():
    ...
    try:
        ...
        portfolio_summary_df = portfolio_summary_df.merge(closing_price_df,
                                                          how='inner',
                                                          on=['symbol_id'])
...
def main(args):
    ...
    try:
        ...
        with PidFile(piddir=tempfile.gettempdir()):
            with multiprocessing.Pool(os.cpu_count(),
                                      custom_logging.logging_worker_init,
                                      [q]) as multipool:
                ...
                if args.daily_update:
                    ...
                    multipool.apply(calculate_fundamental_analysis_ratios,
                                    (TTD_JMD, TTD_USD, TTD_BBD))
                    multipool.apply(update_dividend_yields,
                                    (TTD_JMD, TTD_USD, TTD_BBD))
                ...
                multipool.join()
                logger.info(
                    os.path.basename(__file__) + " executed successfully.")
                q_listener.stop()
        return 0
    except Exception:
