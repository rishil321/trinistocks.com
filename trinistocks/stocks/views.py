...
class HomePageView(ExportMixin, tables2.MultiTableMixin, FilterView):
        ...
        for parameter in required_parameters:
            try:
                ...
                query_string = urlencode({
                    "date":
                    stocks_template_tags.get_latest_date_dailytradingsummary(),
                    "was_traded_today":
                    1,
                    "sort":
                    "-value_traded",
    ...
    def get_context_data(self, *args, **kwargs):
        try:
            ...
            selected_date = datetime.strptime(self.request.GET.get("date"),
                                              "%Y-%m-%d")
            ...
            daily_trading_summary_records = (
                models.DailyStockSummary.objects.filter(
                    was_traded_today=1).filter(date=selected_date).
                select_related("symbol").order_by("-value_traded"))
            ...
            graph_symbols = [
                record["symbol"] for record in selected_records[:10]
            ]
            graph_value_traded = [
            ...
            for record in selected_records:
                if (record["symbol"]
                        not in graph_symbols) and record["value_traded"]:
            ...
            market_indexes_records = (
                models.HistoricalIndicesInfo.objects.filter(
                    date__gt=trailing_30d_date).filter(
            ...
            tnt_dates = [
                datetime.strftime(obj.date, "%d-%m-%Y") for obj in tnt_data
            ]
            composite_data = market_indexes_records.filter(
                index_name="Composite Totals")
            composite_dates = [
                datetime.strftime(obj.date, "%d-%m-%Y")
                for obj in composite_data
            ]
            ...
            sme_dates = [
                datetime.strftime(obj.date, "%d-%m-%Y") for obj in sme_data
            ]
            ...
            context[
                "daily_traded_table"] = stocks_tables.DailyTradingSummaryTable(
                    daily_trading_summary_records)
            ...
            LOGGER.warning("Got a value error while loading this page" +
                           str(verr))
...
class DailyTradingSummaryView(ExportMixin, tables2.views.SingleTableMixin,
                              FilterView):
    """
    ...
    def get(self, request, *args, **kwargs):
        ...
        for parameter in required_parameters:
            try:
                ...
                # if we are missing any parameters, return a redirect
                base_url = reverse("stocks:dailytradingsummary",
                                   current_app="stocks")
                query_string = urlencode({
                    "date":
                    stocks_template_tags.get_latest_date_dailytradingsummary(),
    ...
    def get_context_data(self, *args, **kwargs):
        try:
            ...
            # as well as their symbols, and order by the highest volume traded
            daily_trading_summary_records = (
                models.DailyStockSummary.objects.exclude(
                    was_traded_today=0).filter(date=selected_date).
                select_related("symbol").order_by("-value_traded"))
            if not daily_trading_summary_records:
                raise ValueError(
...
class ListedStocksView(ExportMixin, tables2.MultiTableMixin, FilterView):
    ...
    def get_context_data(self, *args, **kwargs):
        try:
            ...
            context["errors"] = ALERTMESSAGE + str(verr)
            LOGGER.warning("Got a valueerror while loading this page" +
                           str(verr))
        except Exception as ex:
            LOGGER.exception(
                "Sorry. Ran into a problem while attempting to load the page: "
...
class TechnicalAnalysisSummary(ExportMixin, tables2.views.SingleTableMixin,
                               FilterView):
    """
...
class FundamentalAnalysisSummary(ExportMixin, tables2.views.MultiTableMixin,
                                 TemplateView):
    """
    Set up the data for the technical analysis summary page
    """
    ...
    def get(self, request, *args, **kwargs):
        ...
        for parameter in required_parameters:
            try:
                ...
                # if we are missing any parameters, return a redirect
                base_url = reverse("stocks:fundamentalanalysis",
                                   current_app="stocks")
...
class StockHistoryView(ExportMixin, tables2.views.SingleTableMixin,
                       FilterView):
    """
    ...
    def get(self, request, *args, **kwargs):
        ...
        # If the required filters are not present, return a redirect
        required_parameters = [
            "symbol", "date__gte", "date__lte", "chart_type", "sort"
        ]
        for parameter in required_parameters:
            try:
                # check that each parameter has a value
                ...
                query_string = urlencode({
                    "symbol":
                    stocks_template_tags.get_session_symbol_or_default(self),
                    "date__gte":
                    stocks_template_tags.get_1_yr_back(),
                    "date__lte":
                    stocks_template_tags.get_today(),
                    "chart_type":
                    "candlestick",
                    "sort":
                    "-date",
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context = super().get_context_data(*args, **kwargs)
        try:
            listed_stocks = models.ListedEquities.objects.all().order_by(
                "symbol")
            ...
            if self.request.GET.get("configure_button"):
                ...
                self.request.session[
                    "entered_start_date"] = self.entered_start_date.strftime(
                        "%Y-%m-%d")
            ...
            if self.selected_chart_type == "line":
                self.historical_records = (self.model.objects.filter(
                    symbol=self.selected_symbol).filter(
                        date__gte=self.entered_start_date).filter(
                            date__lte=self.entered_end_date).order_by(
                                self.order_by))
                context["graph_labels"] = [
                    obj.date for obj in self.historical_records
                ]
                ...
                context["open_prices"] = [
                    float(num)
                    for num in selected_records.values_list("open_price",
                                                            flat=True)
                ]
                context["close_prices"] = [
                    float(num)
                    for num in selected_records.values_list("close_price",
                                                            flat=True)
                ]
                context["lows"] = [
                    float(num)
                    for num in selected_records.values_list("low", flat=True)
                ]
                context["highs"] = [
                    float(num)
...
class DividendHistoryView(FilterView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        # If the required filters are not present, return a redirect
        required_parameters = [
            "symbol", "record_date__gte", "record_date__lte", "sort"
        ]
        for parameter in required_parameters:
            try:
                # check that each parameter has a value
                ...
                # if we are missing any parameters, return a redirect
                base_url = reverse("stocks:dividendhistory",
                                   current_app="stocks")
                query_string = urlencode({
                    "symbol":
                    stocks_template_tags.get_session_symbol_or_default(self),
                    "record_date__gte":
                    stocks_template_tags.get_5_yr_back(),
                    "record_date__lte":
                    stocks_template_tags.get_today(),
                    "sort":
                    "-record_date",
        ...
        try:
            ...
            self.historical_dividends_paid = (
                models.HistoricalDividendInfo.objects.filter(
                    symbol=self.selected_symbol).filter(
                        record_date__gte=self.entered_start_date).filter(
                            record_date__lte=self.entered_end_date).order_by(
                                self.order_by))
            self.historical_dividend_yields = (
                models.HistoricalDividendYield.objects.filter(
                    symbol=self.selected_symbol).filter(
                        date__gte=self.entered_start_date).filter(
                            date__lte=self.entered_end_date).order_by("-date"))
            # Set up our graph
            self.graph_labels_1 = [
                obj.record_date for obj in self.historical_dividends_paid
            ]
            self.graph_dataset_1 = [
                float(obj.dividend_amount)
                for obj in self.historical_dividends_paid
            ]
            self.graph_labels_2 = [
                obj.date for obj in self.historical_dividend_yields
            ]
            self.graph_dataset_2 = [
                float(obj.dividend_yield)
...
class MarketIndexHistoryView(ExportMixin, tables2.views.SingleTableMixin,
                             FilterView):
    """
    ...
    def get(self, request, *args, **kwargs):
        ...
        for parameter in required_parameters:
            try:
                ...
                # if we are missing any parameters, return a redirect
                base_url = reverse("stocks:marketindexhistory",
                                   current_app="stocks")
                query_string = urlencode({
                    "index_name":
                    "Composite Totals",
    ...
    def get_context_data(self, *args, **kwargs):
        try:
            ...
            if self.request.GET.get("configure_button"):
                ...
                # store the date as a session variable to be reused
                self.request.session[
                    "entered_start_date"] = entered_start_date.strftime(
                        "%Y-%m-%d")
            ...
            if self.request.GET.get("configure_button"):
                entered_end_date = datetime.strptime(
                    self.request.GET.get("date__lte"), "%Y-%m-%d")
                self.request.session[
                    "entered_end_date"] = entered_end_date.strftime("%Y-%m-%d")
            ...
            if self.index_name_needed:
                self.historical_records = (self.model.objects.filter(
                    date__gt=self.entered_start_date).filter(
                        date__lte=self.entered_end_date).filter(
                            index_name=self.index_name).order_by(
                                self.order_by))
            else:
                self.historical_records = (self.model.objects.filter(
                    date__gt=self.entered_start_date).filter(
    ...
    def set_graph_dataset(self, ):
        self.graph_dataset = [
            float(obj[self.index_parameter])
            for obj in self.historical_records.values()
        ]


class OSTradesHistoryView(ExportMixin, tables2.views.SingleTableMixin,
                          FilterView):
    """
    ...
    def get(self, request, *args, **kwargs):
        ...
        for parameter in required_parameters:
            try:
                ...
                # if we are missing any parameters, return a redirect
                base_url = reverse("stocks:ostradeshistory",
                                   current_app="stocks")
                query_string = urlencode({
                    "symbol":
                    stocks_template_tags.get_session_symbol_or_default(self),
    ...
    def get_context_data(self, *args, **kwargs):
        try:
            ...
            if self.symbol_needed:
                ...
                context[
                    "selected_stock_name"] = self.selected_stock.security_name
                context["selected_stock_symbol"] = self.selected_stock.symbol
            if self.index_name_needed:
                context["index_parameter"] = self.index_parameter
                context["index_parameter_string"] = self.index_parameter_string
...
class FundamentalHistoryView(ExportMixin, tables2.views.SingleTableMixin,
                             FilterView):
    """
    ...
    def get(self, request, *args, **kwargs):
        ...
        for parameter in required_parameters:
            try:
                ...
                # if we are missing any parameters, return a redirect
                base_url = reverse("stocks:fundamentalhistory",
                                   current_app="stocks")
                query_string = urlencode({
                    "symbol1":
                    stocks_template_tags.get_session_symbol_or_default(self),
    ...
    def get_context_data(self, *args, **kwargs):
        try:
            ...
            if self.request.GET.get("indicator"):
                ...
                # store the session variable
                self.request.session[
                    "selected_indicator"] = self.selected_indicator
            ...
            historical_records_1 = (self.model.objects.filter(
                symbol=self.symbol1).filter(
                    date__gte=self.entered_start_date).filter(
                        date__lte=self.entered_end_date).filter(
                            report_type="annual"))
            historical_records_2 = (self.model.objects.filter(
                symbol=self.symbol2).filter(
                    date__gte=self.entered_start_date).filter(
                        date__lte=self.entered_end_date).filter(
                            report_type="annual"))
            ...
            historical_close_prices_1 = (
                models.DailyStockSummary.objects.filter(
                    symbol=self.symbol1).filter(
                        date__gte=self.entered_start_date).filter(
                            date__lte=self.entered_end_date))
            ...
            quarterly_dates_1 = [
                obj.date for obj in historical_quarterly_records_1
            ]
            quarterly_dates_2 = [
                obj.date for obj in historical_quarterly_records_2
            ]
            quarterly_dataset_1 = [
                obj[self.selected_indicator]
            ...
            context[
                "selected_indicator_verbose_name"] = self.selected_indicator_verbose_name
...
class StockNewsHistoryView(ExportMixin, tables2.views.SingleTableMixin,
                           FilterView):
    """
...
class RegisterPageView(FormView):
    ...
    def form_valid(self, form, **kwargs):
        ...
        try:
            ...
            get_user_model().objects.create_user(new_username, new_email,
                                                 new_password)
...
class PortfolioTransactionsView(LoginRequiredMixin, FormView):
    ...
    success_url = reverse_lazy("stocks:portfoliotransactions",
                               current_app="stocks")
    ...
    def form_valid(self, form, **kwargs):
        ...
        try:
            ...
            if form.data["bought_or_sold"] == "Sold":
                remaining_shares = (
                    models.PortfolioSummary.objects.all().filter(
                        user_id=current_user.id,
                        symbol_id=form.data["symbol"])[0].shares_remaining)
            ...
            transaction = models.PortfolioTransactions.objects.create(
                user=current_user,
                date=form.data["date"],
                symbol=models.ListedEquities.objects.get(
            ...
            context[
                "general_error"] = "Sorry. It seems like that's a duplicate entry. Did you already add this transaction?"
...
class PortfolioSummaryView(LoginRequiredMixin, ExportMixin,
                           tables2.views.SingleTableMixin, FilterView):
    """
        ...
        try:
            ...
            if "delete" in self.request.GET:
                ...
                if delete_request == "ALL":
                    ...
                    models.PortfolioTransactions.objects.filter(
                        user=current_user).filter(
                            symbol_id=delete_request).delete()
                    delete_request_message = (
...
class PasswordResetRequestView(FormView):
    ...
    success_url = reverse_lazy("stocks:password_reset_request",
                               current_app="stocks")
...
class DailyStocksTradedApiView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        return (models.DailyStockSummary.objects.exclude(
            was_traded_today=0).filter(date=filter_date).select_related(
                "symbol").order_by("-value_traded"))
